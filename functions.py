...
def PrintTable(table):
    ...
    print("{:>5} {:<5} {:<5} {:<5} {:<5}".format(
        "NT",
        '-' if table.contents.resTable[4][0] <= 6 else table.contents.resTable[4][0] - 6,
        '-' if table.contents.resTable[4][2] <= 6 else table.contents.resTable[4][2] - 6,
        '-' if table.contents.resTable[4][1] <= 6 else table.contents.resTable[4][1] - 6,
        '-' if table.contents.resTable[4][3] <= 6 else table.contents.resTable[4][3] - 6))
    for suit in range(0, dds.DDS_SUITS):
        print("{:>5} {:<5} {:<5} {:<5} {:<5}".format(
            hands.dcardSuit[suit],
            '-' if table.contents.resTable[suit][0] <= 6 else table.contents.resTable[suit][0] - 6,
            '-' if table.contents.resTable[suit][2] <= 6 else table.contents.resTable[suit][2] - 6,
            '-' if table.contents.resTable[suit][1] <= 6 else table.contents.resTable[suit][1] - 6,
            '-' if table.contents.resTable[suit][3] <= 6 else table.contents.resTable[suit][3] - 6))
    print("")
...
#  for h in range(dds.DDS_HANDS):
#  if h == 0:
#  offset = DDS_HAND_OFFSET + 1
#  line = 0
#  elif h == 1:
#  offset = DDS_HAND_OFFSET * 2 + 1
#  line = 4
#  elif h == 2:
#  offset = DDS_HAND_OFFSET + 1
#  line = 8
